...
void CMSIS_SPI1_init(void) {
	...
	//SCK - PA5:
	MODIFY_REG(GPIOA->CRL, GPIO_CRL_MODE5, 0b00 << GPIO_CRL_MODE5_Pos); //Input
	MODIFY_REG(GPIOA->CRL, GPIO_CRL_CNF5, 0b01 << GPIO_CRL_CNF5_Pos); //Input floating
	//MISO - PA6:
	MODIFY_REG(GPIOA->CRL, GPIO_CRL_MODE6, 0b11 << GPIO_CRL_MODE6_Pos); //Alternative Function output
	MODIFY_REG(GPIOA->CRL, GPIO_CRL_CNF6, 0b10 << GPIO_CRL_CNF6_Pos); //Push-pull
	//MOSI - PA7:
	MODIFY_REG(GPIOA->CRL, GPIO_CRL_MODE7, 0b00 << GPIO_CRL_MODE7_Pos); //Input
	MODIFY_REG(GPIOA->CRL, GPIO_CRL_CNF7, 0b01 << GPIO_CRL_CNF7_Pos); //Input floating
	...
	CLEAR_BIT(SPI1->CR1, SPI_CR1_SSI); //0: Software slave management disabled
	CLEAR_BIT(SPI1->CR1, SPI_CR1_MSTR); //0: Slave configuration 
	...
	SET_BIT(SPI1->CR2, SPI_CR2_RXNEIE); //0: RXNE interrupt masked 
	...
	NVIC_EnableIRQ(SPI1_IRQn); //Включим прерывания по spi1
